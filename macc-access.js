...
(function(){
  ...
  async function loadAccessData(){
    ...
    memberEl.innerHTML=(members.data||[]).map(m=>`<div style="padding:9px 0;border-bottom:1px solid var(--border);display:flex;gap:8px;align-items:center;justify-content:space-between"><div><div style="font-size:12px;color:var(--text)">${esc(m.email)}</div><div style="font-size:10px;color:var(--text3)">${roleLabel(m.role)} · ${m.active?'активний':'доступ забрано'}</div></div>${m.id!==session.user.id&&m.active?`<button class="btn sm danger" onclick="maccRevokeUser('${m.id}','${esc(m.email)}')">Забрати доступ</button>`:''}</div>`).join('')||'<div class="macc-access-note">Користувачів ще немає.</div>';
    const records=[...(access.data||[]).map(x=>({at:x.created_at,text:x.event==='login'?'Вхід до сайту':'Вихід із сайту'})),...(audit.data||[]).map(x=>({at:x.created_at,text:x.action+(x.details?.sections?.length?' — '+x.details.sections.join(', '):'')}))].sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,16);
    historyEl.innerHTML=records.map(x=>`<div style="padding:8px 0;border-bottom:1px solid var(--border);font-size:12px;color:var(--text2)"><span style="color:var(--text3);font-size:10px">${new Date(x.at).toLocaleString('uk-UA')}</span><br>${esc(x.text)}</div>`).join('')||'<div class="macc-access-note">Ще немає записів.</div>';
  }
  async function invite(){
    const email=document.getElementById('invite-email').value.trim(), role=document.getElementById('invite-role').value, out=document.getElementById('invite-result');
