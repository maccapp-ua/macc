...
(function(){
  ...
  async function loadAccessData(){
    ...
    const records=[...(access.data||[]).map(x=>({at:x.created_at,by:emails[x.user_id]||'—',text:x.event==='login'?'Вхід до сайту':'Вихід із сайту'})),...(audit.data||[]).map(x=>({at:x.created_at,by:emails[x.user_id]||'—',text:x.action+(x.details?.sections?.length?' — '+x.details.sections.join(', '):'')}))].sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,50);
